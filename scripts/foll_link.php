...
WHERE `room_id` = '" . $_REQUEST['room_id'] . "'";
    ...
    echo '<div class="infinity-slider-wrap">
    			<div class="prev n"><a href="#">&laquo;</a></div>
				<div class="infinity-slider">';
    while ($img_result = mysqli_fetch_assoc($img_room_arr)) {
        $img = base64_encode($img_result['img']);
        echo '<img src="data:img/jpeg;base64, ' . $img . '" alt="Фото">';
    }
    echo '</div>
    			<div class="next n"><a href="#">&raquo;</a></div>
    			</div>';
    mysqli_close($connect);
